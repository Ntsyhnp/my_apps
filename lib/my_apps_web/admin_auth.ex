defmodule MyAppsWeb.AdminAuth do
  use MyAppsWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_my_apps_web_admin_remember_me"
  ...
  def log_out_admin(conn) do
    ...
    admin_token && AdminPanel.delete_admin_session_token(admin_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      MyAppsWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_admin(conn, _opts) do
    ...
    admin = admin_token && AdminPanel.get_admin_by_session_token(admin_token)
  ...
  the current_admin:
      ...
      defmodule MyAppsWeb.PageLive do
        use MyAppsWeb, :live_view

        on_mount {MyAppsWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{MyAppsWeb.AdminAuth, :ensure_authenticated}] do
  ...
  defp mount_current_admin(socket, session) do
    Phoenix.Component.assign_new(socket, :current_admin, fn ->
      if admin_token = session["admin_token"] do
        AdminPanel.get_admin_by_session_token(admin_token)
      end
    end)
  end
